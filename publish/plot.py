...
def plot(y_key, x_key, ylogscale = False):
    ...
    ax_legend.legend(
        ...
        loc='lower left', 
        bbox_to_anchor=(legend_margin, 1.1),
        borderaxespad=0,
        fontsize="20"
    )
    # ax_legend.legend(
...
def plot_diversity_time(index, ykey, xkey ,ylogscale = False, xlogscale = False):
    ...
    for gs, dataset_name in zip(grid_specs, results):
        ...
        for alg,result in results[dataset_name].items():
            ...
            if xlogscale:
                ax.set_xscale('log')
            ax.set_xlabel(set_x_label, fontsize="16")
            #ax.set_title(f'({alp[i]}) {dataset_name}', y = -0.4, fontsize="20")
            # ax.set_xticks([20, 40, 60, 80, 100])
            # ax.tick_params(axis='both', which='major', labelsize=18)
        # plot the skyline dotted line
        ax.set_title(f'({alp[alp_i]}) {dataset_name}', y = -0.4, fontsize="20")
        alp_i = alp_i + 1
...
plot_dir = result_file_dir + '/' + result_file_name + '/missing_points'
if not os.path.exists(plot_dir):
   os.mkdir(plot_dir)
