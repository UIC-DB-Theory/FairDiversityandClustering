    ...
    for k in range(setup["parameters"]["k"][0] ,setup["parameters"]["k"][1], setup["parameters"]["k"][2]):
        ...
        for obs in range(0, setup['parameters']['observations']):
            ...
            for name in setup['algorithms']:
                ...
                if name in gurobiUsers:
                    print(f'\t\t\tSkipping {name} that uses Gurobi...')
                    continue
...
# Add cached results for gurobi using algs
for alg in gurobiUsers:
    if alg in setup['algorithms']:
